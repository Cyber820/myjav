...
function ensureStyles() {
  ...
  const style = el('style', { id: 'af-video-modal-style', html: `
    ...
    .af-grid{display:grid;grid-template-columns:180px 1fr;gap:10px 12px;align-items:start;}
    .af-label{font-size:13px;color:rgba(0,0,0,.75);padding-top:8px;}
...
function validate(form, { publisherSel, actressSel, actressTypeSel, costumeSel, sceneSel, tagSel }) {
  ...
  if (!video_name) errors.push('影片名称必填。')
  ...
  else errors.push('有码必选：请选择“有码”或“无码”。')
  ...
  return {
    ...
    payload: {
      ...
      links: { actressIds, actressTypeIds, costumeIds, sceneIds, tagIds },
    },
  }
}
...
export function openVideoCreateModal({ onCreated } = {}) {
  ...
  const censoredSel = el('select', { class: 'af-select', name: 'censored' }, [
    el('option', { value: '' }, [document.createTextNode('请选择…')]),
    el('option', { value: 'true' }, [document.createTextNode('有码')]),
    el('option', { value: 'false' }, [document.createTextNode('无码')]),
  ])

  const hasSpecialSel = el('select', { class: 'af-select', name: 'has_special' }, [
    el('option', { value: '' }, [document.createTextNode('（可选）')]),
    el('option', { value: 'true' }, [document.createTextNode('是')]),
    el('option', { value: 'false' }, [document.createTextNode('无')]),
  ])

  // “猎奇内容”整行：label + input 一起隐藏/显示
  const specialArea = el('textarea', { class: 'af-textarea big', name: 'special', placeholder: '仅当选择“是”时填写' })
  const specialLabel = el('div', { class: 'af-label' }, [document.createTextNode('猎奇内容')])
  const specialField = el('div', {}, [specialArea])
  const specialLabelWrap = el('div', { class: 'af-rowhide', id: 'af-special-label-wrap' }, [specialLabel])
  const specialFieldWrap = el('div', { class: 'af-rowhide', id: 'af-special-field-wrap' }, [specialField])

  function syncSpecialVisibility() {
    const show = norm(hasSpecialSel.value) === 'true'
    if (show) {
      specialLabelWrap.classList.remove('af-rowhide')
      specialFieldWrap.classList.remove('af-rowhide')
    } else {
      specialLabelWrap.classList.add('af-rowhide')
      specialFieldWrap.classList.add('af-rowhide')
      specialArea.value = ''
    }
  }
  ...
  const secRates = el('div', { class: 'af-section' }, [
    ...
    el('div', { class: 'af-grid' }, [
      ...
      el('div', { class: 'af-label' }, [document.createTextNode('情节')]),
      el('textarea', { class: 'af-textarea big', name: 'storyline', placeholder: '可选' }),

      el('div', { class: 'af-label' }, [document.createTextNode('猎奇')]),
      hasSpecialSel,
      ...
      el('div', { class: 'af-label' }, [document.createTextNode('个人点评')]),
