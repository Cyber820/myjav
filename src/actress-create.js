...
function ensureStyles() {
  ...
  const style = el('style', { id: 'af-actress-modal-style', html: `
    ...
    .af-modal{width:min(720px,100%);max-height:85vh;overflow:auto;background:#fff;border:1px solid rgba(0,0,0,.2);border-radius:12px;padding:14px;box-sizing:border-box;}
    ...
    .af-hint{font-size:12px;color:rgba(0,0,0,.6);}
    .af-error{margin-top:10px;border:1px solid rgba(200,0,0,.25);background:rgba(200,0,0,.06);padding:10px;border-radius:10px;color:#7a0000;font-size:13px;white-space:pre-wrap;}
...
function validate(form) {
  ...
  // <input type="date"> 输出通常是 YYYY-MM-DD，直接可写入 Postgres date
...
export function openActressCreateModal({ onCreated } = {}) {
  ...
  const errorBox = el('div', { class: 'af-error', style: 'display:none;' })
  ...
  const btnSubmit = el('button', { class: 'af-btn af-btn-primary', type: 'submit', html: '提交' })
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (error) {
      errorBox.style.display = 'block'
      errorBox.textContent = `写入失败：${error.message}`
      return
    }
